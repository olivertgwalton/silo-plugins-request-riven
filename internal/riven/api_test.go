...
func TestCompletedItemIDs(t *testing.T) {
	stub, client := newStub(t, `{"data":{"itemsByState":[{"id":1},{"id":42}]}}`)

	ids, err := CompletedItemIDs(context.Background(), client, "MOVIE")
	if err != nil {
		t.Fatalf("CompletedItemIDs: %v", err)
	}
	...
	if vars["state"] != "Completed" || vars["itemType"] != "MOVIE" {
		t.Fatalf("bad variables: %+v", vars)
	}
}

func TestCompletedItemIDsEmpty(t *testing.T) {
	_, client := newStub(t, `{"data":{"itemsByState":[]}}`)

	ids, err := CompletedItemIDs(context.Background(), client, "EPISODE")
	if err != nil {
		t.Fatalf("CompletedItemIDs: %v", err)
	}
	if len(ids) != 0 {
...
func TestFilesystemPaths(t *testing.T) {
	stub, client := newStub(t, `{"data":{"filesystemEntries":[{"path":"/mount/movies/Arrival (2016)/Arrival.mkv"}]}}`)

	paths, err := FilesystemPaths(context.Background(), client, 42)
	if err != nil {
		t.Fatalf("FilesystemPaths: %v", err)
	}
	if len(paths) != 1 || paths[0] != "/mount/movies/Arrival (2016)/Arrival.mkv" {
...
func TestFilesystemPathsGraphQLError(t *testing.T) {
	_, client := newStub(t, `{"errors":[{"message":"not found"}]}`)

	_, err := FilesystemPaths(context.Background(), client, 1)
